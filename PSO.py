...
class PSO(Optimiser):
    ...
    def __init__(self, constraints, swarm,
        ...
        self.nbhd_topology = nbhd_toplogy
        ...
        if v_start_delta > 0:
            tmp_V = self.rng.normal(size=(self.swarm_size, self.dimensions))
            tmp_V = tmp_V / np.linalg.norm(tmp_V, axis=1)[:,np.newaxis]
    ...
    def _update_best(self):
        ...
        elif self.nbhd_toplogy == 'lbest':
            raise NotImplementedError
            self.sbest = np.zeros(self.swarm_size)
            self.sbest[:len(self.swarm_size)-1]
    ...
    def _update_velocity(self):
        ...
        if self.variant == 'constrict':
            ...
            V_temp = self.params['chi'] * (self.V + self.params['c1'] * r1 * (self.pbest - self.X) + self.params['c2'] * r2 * (self.sbest - self.X))
