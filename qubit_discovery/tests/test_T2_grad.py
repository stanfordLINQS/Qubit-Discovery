...
UNIT_FACTORS = {
    Capacitor: lambda EC: sqf.numpy(-unt.e**2/2/sq.Capacitor(EC, 'GHz')._value**2/(2*np.pi*unt.hbar) / 1e9),
    Inductor: lambda EL: sqf.numpy(-(unt.Phi0/2/np.pi)**2/sq.Inductor(EL, 'GHz')._value**2/(2*np.pi*unt.hbar)/1e9),
    Junction: 1 / 2 / np.pi / 1e9
}

def do_grad_func_test(cr: Circuit, 
                   cr_delta: Circuit,
                   delta,
                   unit_factor,
                   np_func,
...
def do_test_dec_rate_fluxonium(
        dec_rate_func, dec_grad,
        EC = 3.6, EL = 0.46, EJ = 20.5,
        delta = 1e-6
    ):
    ...
    for optim_mode in [True, False]:
        ...
        for flux_point in flux_points:
            ...
            cr_delta_J  = build_fl_elements(EC, EL, EJ + delta, flux_point, 'junctions')
            cr_delta_L = build_fl_elements(EC, EL + delta, EJ, flux_point, 'junctions')
            cr_delta_C = build_fl_elements(EC + delta, EL, EJ, flux_point, 'junctions')
...
def do_test_torch_func(cr: Circuit, 
                    cr_delta: Circuit,
                    delta,
                    unit_factor,
                    np_func,
...
def do_test_dec_torch_trans(
    ...
    for charge_offset in charge_points:
        ...
        cr_delta_C = build_trans_elements(EC + delta, EJ, flux_point, 'junctions')
        cr_delta_J = build_trans_elements(EC, EJ + delta, flux_point, 'junctions')
